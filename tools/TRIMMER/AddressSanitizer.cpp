...
// set filename of Debug Location with the help of existing instruction
...
// set filename of the debug location with the help of function debug metadata
...
// bundle function to handle different cases
                                            ...
                                            const std::string &newFilename, const Function *F = nullptr)
{
  ...
  errs() << "Warning: Cannot clone debug location with new filename: " << newFilename << "\n";
  return DebugLoc();
}
...
private:
  ...
  Function *getDebugFunc() {
    if (AsanHandleNoReturnFunc)
        return AsanHandleNoReturnFunc;
    if (AsanPtrCmpFunction)
        return AsanPtrCmpFunction;
    if (AsanPtrSubFunction)
        return AsanPtrSubFunction;
    if (AsanMemmove)
        return AsanMemmove;
    if (AsanMemcpy)
        return AsanMemcpy;
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            for (size_t k = 0; k < kNumberOfAccessSizes; k++) {
                if (AsanErrorCallback[i][j][k])
                    return AsanErrorCallback[i][j][k];
                if (AsanMemoryAccessCallback[i][j][k])
                    return AsanMemoryAccessCallback[i][j][k];
            }
            if (AsanErrorCallbackSized[i][j])
                return AsanErrorCallbackSized[i][j];
            if (AsanMemoryAccessCallbackSized[i][j])
                return AsanMemoryAccessCallbackSized[i][j];
        }
    }
...
static void instrumentMaskedLoadOrStore(AddressSanitizer *Pass,
  ...
  // Debloat end
    ...
    // Debloat
    DebugLoc newLoc =
        cloneDebugLocWithNewFilename(InsertBefore, "AddressSanitizer.cpp_6");
    IRB.SetCurrentDebugLocation(newLoc);
    // Debloat end
...
Instruction *AddressSanitizer::generateCrashCode(Instruction *InsertBefore,
  ...
  // Debloat
  //DebugLoc newLoc = cloneDebugLocWithNewFilename(
  //    InsertBefore, "AsanErrorCallback_AddressSanitizer.cpp_9", getDebugFunc());
  //IRB.SetCurrentDebugLocation(newLoc);
  // Debloat end
...
// Debloat
  DebugLoc newLoc = cloneDebugLocWithNewFilename(
      InsertBefore, "Instrumentation_AddressSanitizer.cpp_12", getDebugFunc());
  IRB.SetCurrentDebugLocation(newLoc);
  // Debloat end
  ...
  // Debloat
    DebugLoc newLoc =
        cloneDebugLocWithNewFilename(TagCheckTerm, "TAddressSanitizer.cpp_13");
    IRB.SetCurrentDebugLocation(newLoc);
    // Debloat end
  ...
  if (ClAlwaysSlowPath || (TypeSize < 8 * Granularity)) {
    ...
    // Debloat
    DebugLoc newLoc =
        cloneDebugLocWithNewFilename(CheckTerm, "AddressSanitizer.cpp_13", getDebugFunc());
    IRB.SetCurrentDebugLocation(newLoc);
    // Debloat end
  ...
  // Debloat
  newLoc = cloneDebugLocWithNewFilename(OrigIns, "AsanErrorCallbackAddressSanitizer.cpp_9", getDebugFunc());
...
IRBuilder<> AddressSanitizerModule::CreateAsanModuleDtor(Module &M) {
  ...
  Instruction *inst_ = ReturnInst::Create(*C, AsanDtorBB);
  IRBuilder<> IrbDtor = IRBuilder<>(inst_);
// Debloat
    for(const Function &f: M.functions()) {
      if(f.getSubprogram()) {
        DebugLoc newLoc = cloneDebugLocWithNewFilename(
        inst_, "Construct_AddressSanitizer.cpp_18", &f);
...
bool AddressSanitizerModule::runOnModule(Module &M) {
  ...
  if (ClGlobals) {
	...
	if(f.getSubprogram()) {
	  DebugLoc newLoc = cloneDebugLocWithNewFilename(
	  AsanCtorFunction->getEntryBlock().getTerminator(), "Construct_AddressSanitizer.cpp_999", &f);
	  IRB.SetCurrentDebugLocation(newLoc);
	}
    }
    // Debloat end
...
bool AddressSanitizer::runOnFunction(Function &F) {
  ...
  for (auto CI : NoReturnCalls) {
    ...
    // Debloat
  DebugLoc newLoc = cloneDebugLocWithNewFilename(
      CI, "Construct_AddressSanitizer.cpp_98");
  IRB.SetCurrentDebugLocation(newLoc);
  // Debloat end
...
// Debloat
  for(const Function &f: M.functions()) {
    if(f.getSubprogram()) {
      DebugLoc newLoc = cloneDebugLocWithNewFilename(
      ReturnInst::Create(*C), "Construct_AddressSanitizer.cpp_97", &f);
      IRB.SetCurrentDebugLocation(newLoc);
    }
  }
  // Debloat end
...
// Debloat
  ...
  if (DoStackMalloc) {
    ...
    // Debloat
    DebugLoc newLoc =
          cloneDebugLocWithNewFilename(InsBefore, "Construct_AddressSanitizer.cpp_32", &F);
    ...
    IRBIf.SetCurrentDebugLocation(EntryDebugLocation);
    // Debloat
    newLoc = cloneDebugLocWithNewFilename(InsBefore, "AddressSanitizer.cpp_93", &F);
    IRB.SetCurrentDebugLocation(newLoc);
    // Debloat end
    ...
    IRB.SetCurrentDebugLocation(EntryDebugLocation);
     // Debloat
      newLoc = cloneDebugLocWithNewFilename(InsBefore, "AddressSanitizer.cpp_94", &F);
      IRB.SetCurrentDebugLocation(newLoc);
      // Debloat end
  ...
  if (!StaticAllocaPoisonCallVec.empty()) {
    ...
    for (const auto &APC : StaticAllocaPoisonCallVec) {
      ...
      // Debloat
      DebugLoc newLoc = cloneDebugLocWithNewFilename(
          APC.InsBefore, "Construct_AddressSanitizer.cpp_36", &F);
      IRB.SetCurrentDebugLocation(newLoc);
      // Debloat end
  ...
  for (auto Ret : RetVec) {
    ...
    // Debloat
    DebugLoc newLoc =
        cloneDebugLocWithNewFilename(Ret, "Construct_AddressSanitizer.cpp_37", &F);
    ...
    if (DoStackMalloc) {
      ...
      // Debloat
      DebugLoc newLoc = cloneDebugLocWithNewFilename(
          ThenTerm, "Construct_AddressSanitizer.cpp_38", &F);
...
// Debloat
      newLoc = cloneDebugLocWithNewFilename(ElseTerm,
                                            "Construct_AddressSanitizer.cpp_39", &F);
